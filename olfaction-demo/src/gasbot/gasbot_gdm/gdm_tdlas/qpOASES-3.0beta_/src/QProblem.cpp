	...
	for (i = 0; i < nV; i++)
	{
		switch (bounds.getType(i))
		{
			...
			case ST_BOUNDED:
				...
				if (bstat != ST_LOWER) { sca = fmax(fabs(x[i]), 1.0); lb[i] = x[i] - sca * rampValP; }
				if (bstat != ST_UPPER) { sca = fmax(fabs(x[i]), 1.0); ub[i] = x[i] + sca * rampValP; }
	...
	for (i = 0; i < nC; i++)
	{
		switch (constraints.getType(i))
		{
			...
			case ST_BOUNDED:
				...
				if (cstat != ST_LOWER) { sca = fmax(fabs(Ax[i]), 1.0); lbA[i] = Ax[i] - sca * rampValP; }
				if (cstat != ST_UPPER) { sca = fmax(fabs(Ax[i]), 1.0); ubA[i] = Ax[i] + sca * rampValP; }
