...
// ######################### Writing log file ##############################
void writeFile(int golS, double golX, double golY, double golW, double posX, double posY, double posW, \
		int statePTU, double anglePan, double angleTilt, int measure){
	if(historyFile){
		...
		// Print info
		ROS_INFO("Goal#%d <%.2f,%.2f,%.2f>; Pose <%.2f,%.2f,%.2f>; PTU Status %d; Pan-Tilt Status <%.2f,%.2f>; Measurement %d;",\
			  golS,golX,golY,golW,posX,posY,posW,statePTU,anglePan,angleTilt,measure);
	}
}
...
// ######################### Callback: Current Goal ##############################
void callbackCurrentGoal(const geometry_msgs::PoseStamped::ConstPtr& goal){
	
	// Goal Sequence Number
	golS = goal->header.seq;

	// Position
	golX = goal->pose.position.x;
	golY = goal->pose.position.y;
...
void callbackRMLD(rmld_node::rmld_msg mea){
	...
	if (measure >= GAS_MEASURE_THRESHOLD && (statePTU==OPERATION_STATUS || OPERATION_STATUS==5)){	
		...
		writeFile(golS,golX,golY,golW,posX,posY,posW,statePTU,anglePan,angleTilt,measure);
	}
}	
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber sub5 = n.subscribe("/move_base/current_goal",1000,callbackCurrentGoal);
